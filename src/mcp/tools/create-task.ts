...
export const CreateTaskArgsSchema = z.object({
  name: z
    ...
    .describe(
      "One-line summary (like GitHub Issue title). Action-oriented, specific. Example: 'Add JWT authentication to API endpoints'",
    ),
  description: z
    .string()
    .min(1)
  ...
  parent_id: z
    ...
    .optional()
    .describe(
      "Parent task ID to create as child. Supports 3-level hierarchy: epic (L0) → task (L1) → subtask (L2). Cannot create children of subtasks (max depth enforced).",
    ),
  priority: z
    .number()
    .int()
    ...
    .optional()
    .describe(
      "Priority level - lower number = higher priority (default: 1, max: 100)",
    ),
  blocked_by: z
    .array(z.string().min(1))
    .optional()
    .describe(
      "Array of task IDs that must be completed before this task can be started. Creates bidirectional blocking relationships.",
    ),
});
